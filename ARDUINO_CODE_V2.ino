...
void setup() {
  ...
  setMaxVolume();
  delay(100);
  Serial.println("Realizando calibração ...");
  calibracao();
  Serial.println("Calibração finzalizada");
  Serial.print("Noise limit value: ");
...
void loop() {
  ...
  if(ESP1.available()){
    ...
    rec_flag = true;
    digitalWrite(RADIO_FLAG, LOW);
    Serial.print("Mensagem chegada do ESP: ");
    Serial.println(req);
  }
  
  if(rec_flag == true){
    for(coleta=0; coleta<2000; coleta++){
      if(analogRead(2)>(noise_limit+10)){
        message_detected = true;
      }
      delay(5);
    }
    if(message_detected == true){
      Serial.println("Mensagem Detectada!");
      delay(3000);
